...
class CustomRoIHead(StandardRoIHead):
    ...
    def forward_train(self,
        ...
        if gt_bboxes2 is not None:
            ...
            for i in range(num_imgs):
                assign_result = self.bbox_assigner.assign(
                    proposal_list[i], gt_bboxes2[i], gt_bboxes_ignore[i],
                    gt_labels2[i])
                sampling_result = self.bbox_sampler.sample(
                    assign_result,
                    proposal_list[i],
                    ...
                    gt_labels2[i],
                    feats=[lvl_feat[i][None] for lvl_feat in x])
                sampling_results2.append(sampling_result)
        else:
            sampling_results2 = sampling_results
        ...
        if self.with_mask:
            mask_results = self._mask_forward_train(x, sampling_results2,
    ...
    def simple_test(self,
        ...
        if not self.with_mask:
            ...
            segm_results = self.simple_test_mask(
                x, img_metas, det_bboxes, det_labels, rescale=rescale, return_iou=return_iou)
