...
class StandardRoIHeadNTM(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def forward_train(self,
        ...
        if self.with_mask:
            mask_results = self._mask_forward_train(x, sampling_results,
                                                    ...
                                                    gt_masks, img_metas, compute_coarse=compute_coarse)
    ...
    def _mask_forward_train(self, x, sampling_results, bbox_feats, gt_masks,
            img_metas, compute_coarse=False):
        """Run forward function and calculate loss for mask head in
        training."""
        if not self.share_roi_extractor:
            pos_rois = bbox2roi([res.pos_bboxes for res in sampling_results])
            mask_results = self._mask_forward(x, pos_rois, compute_coarse=compute_coarse)
        else:
            pos_inds = []
            ...
            mask_results = self._mask_forward(
                x, pos_inds=pos_inds, bbox_feats=bbox_feats, compute_coarse=compute_coarse)

        mask_targets = self.mask_head.get_targets(sampling_results, gt_masks, self.train_cfg)
        if mask_results['mask_pred_coarse'] is not None:
    ...
    def _mask_forward(self, x, rois=None, pos_inds=None, bbox_feats=None, compute_coarse=False):
        ...
        mask_pred, mask_pred_coarse = self.mask_head(mask_feats, compute_coarse=compute_coarse)
        mask_results = dict(mask_pred=mask_pred, mask_pred_coarse=mask_pred_coarse, mask_feats=mask_feats)
        return mask_results
...
@HEADS.register_module()
class CustomRoIHead(StandardRoIHeadNTM):
